...
class MarkItDown:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # magika 已移除 (避免 onnxruntime 重依赖，用纯 mimetypes 替代)
    ...
    def _get_stream_info_guesses(
        ...
        using extension and mimetype (magika removed to avoid onnxruntime).
        """
        guesses: List[StreamInfo] = []
        ...
        # For text files, try charset detection
        cur_pos = file_stream.tell()
        try:
            if enhanced_guess.mimetype and enhanced_guess.mimetype.startswith("text/"):
                stream_page = file_stream.read(4096)
                charset_result = charset_normalizer.from_bytes(stream_page).best()
                if charset_result is not None:
                    charset = self._normalize_charset(charset_result.encoding)
                    enhanced_guess = enhanced_guess.copy_and_update(charset=charset)
        finally:
            file_stream.seek(cur_pos)
